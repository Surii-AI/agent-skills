...
5. If approved, integrate with deterministic Git. Confirm the ticket head is an ancestor of the integration head, then run the required smoke checkpoint — eliding it only when the worker-verified and integration tree hashes are equal, per the elision rule in `references/verification-policy.md`, recording the equal tree hashes as checkpoint evidence.
