...
4. Route by review tier per `references/verification-policy.md`: guided tickets render `templates/reviewer-prompt.md` (plan-aware reviewer receives the guidance and labels each blocking finding `IMPLEMENTATION` or `PLAN`, routing repair to the junior or back to the senior guide); unguided low-risk tickets that cannot skip review render `templates/quick-reviewer-prompt.md` on the fast tier, and an `ESCALATE` verdict re-renders the full reviewer prompt once with the guidance pointer empty.
