...
| Apprentice implementer | Lowest thinking tier (flash at low) | Mechanical direct-dispatch assignment (templates/apprentice-prompt.md); falls back to the junior when absent. |
...
When the environment exposes per-task model selection, map the tiers explicitly — for example `opus`-class for the senior roles and `haiku`-class for the junior, or GLM-5.3 at `:high` thinking for the senior roles and GLM-5.3-flash at `:high` for the junior and the quick reviewer (high, not low: the junior must still notice when the code proves a plan step wrong — `tdd-apprentice` is the deliberate exception, running low and stopping at the first doubt instead of reasoning). When it does not (OMP's task tool today exposes agent types, not per-task models), run both senior and junior roles on the default `task` agent and use `reviewer` for the plan-aware review: the pair still earns its keep through context isolation, because the guide's investigation never enters the junior's context window and the plan is re-derived from repository evidence instead of conversation memory.

On OMP, per-dispatch models are expressed through the installed agent definitions rather than dispatch flags: `tdd-senior` and `tdd-reviewer` pin GLM-5.3 `:high` with read-only tool sets, and `tdd-junior` pins GLM-5.3-flash `:high` (the 5.3 model family has no `medium` thinking variant); `tdd-quick-reviewer` likewise pins GLM-5.3-flash `:high` with a read-only tool set. Without them, default to `task` + `reviewer` as above. Each definition also carries an explicit `thinking: high` field: the model selector's `:high` suffix alone leaves effort resolution to model defaults, and some models default to `max` — the explicit field pins `high` regardless of the model's default effort. When authoring or editing a `tdd-*` definition, keep both pins in agreement. `tdd-apprentice` pins GLM-5.3-flash `:low` with `thinking: low` — the one role whose low pin is the point.
...
| `tdd-apprentice` | GLM-5.3-Flash | low | Read, Write, Edit, Bash, Grep, Glob | — |
| `tdd-reviewer` | GLM-5.3 | high | Read, Grep, Glob, Bash | — |
| `tdd-quick-reviewer` | GLM-5.3-Flash | high | Read, Grep, Glob, Bash | — |

ZCode's reasoning variants are low, high, and max — matching the GLM-5.3 model family, which also has no `medium`, so the omp junior pin `flash:high` carries over as ZCode thoughtLevel `high`, never low — except `tdd-apprentice`, whose low pin is the point: the junior and the quick reviewer must notice what the apprentice is allowed to stop on. Omitting the subagent and skill tools from each profile makes the no-recursion invariant structural.
...
**Dispatch-failure ladder.** Whenever a dispatch fails with a transient or environment error (e.g. a provider rate limit), degrade one rung at a time and record each rung in the ledger (`scripts/run_state.py record --kind ruling`): (1) retry the dispatch once after a short wait; (2) fall back one model tier (a pinned `tdd-*` agent → the default `task` agent) and retry; (3) only then degrade to controller-sequential execution. When the failure reports a reset time (e.g. 429 with a reset timestamp), record it and re-probe with one minimal read-only dispatch at each new wave instead of committing to degraded mode for the whole run.

**Degraded-mode obligations.** If the environment cannot launch subagents at all, controller-sequential execution still retains the ticket graph, integration worktree, risk gates, pairing decisions (the controller writes the base-pinned guidance document before implementing from it), state ledger, and context boundaries — and additionally records every checkpoint outcome via `scripts/run_state.py checkpoint`, runs reviews controller-side with hand-computed oracles marked not-independent in the review file, and opens the run with a ledger ruling naming the degradation cause and the reset time if known. Report that concurrency was unavailable; do not fake parallelism.
