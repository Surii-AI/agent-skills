...
1. **Discover** the canonical commands from repo tooling: task-runner targets (`justfile`, `Taskfile`), package scripts (`package.json`, `Cargo.toml`, `pyproject.toml`), compose or container configuration, and CI workflows (the service setup steps a CI job runs are the most reliable source of truth). Discover code-location tooling in the same pass: a `.codegraph/` directory (verify the `codegraph` binary is on PATH and run `codegraph sync` once — a stale index misroutes every worker) or an equivalent locator rule in the repository's instruction files. With a CodeGraph index, also resolve the `codegraph-cli` skill file the way the `i-have-adhd` dependency resolves (`references/adapters.md`) and record the locator in run state — binary, index path, resolved skill path — because that record renders into every brief's Code locator section (`references/ticket-format.md`). The skill is a detail pointer, never a run-stopper: an absent skill or absent index just leaves grep/find as the locator. `.codegraph/` is usually untracked, so fresh worktrees do not inherit it — when the index lives only in the user checkout, record that path too, and workers in unindexed workspaces query it with `-p <indexed-checkout>` instead of silently losing the locator.
...
Scope the per-ticket smoke deterministically: prefer the repository's own affected-test selector discovered at preflight and recorded in run state (CodeGraph: `codegraph affected <changed-files>` — or `git diff --name-only | codegraph affected --stdin` — where preflight found an index; vitest: `vitest run --changed <base>` or `pnpm --filter <owner-package> exec vitest run <files>`; turbo/nx: `affected`; Go: package paths); otherwise map the diff's changed paths to their owning workspace packages and run those packages' suites. Per-integration judgment calls are controller tokens and variance; a selector is a command.

## Review and repair
Give a ticket reviewer the ticket, risk, the senior guidance when the ticket was guided, implementer report, packaged base-to-head diff, relevant specification pointer, the interface notes of any blockers it builds on, and scoped repository instructions. Require evidence tied to acceptance criteria. Do not ask it to rediscover the full codebase or rerun the full suite without a concrete doubt. An independent oracle is sized to the acceptance criteria — a few hand-computed cases per criterion, roughly a dozen total; a larger sweep only when a defect is already suspected in a numeric criterion; never a re-derivation of the implementation's own test matrix.
