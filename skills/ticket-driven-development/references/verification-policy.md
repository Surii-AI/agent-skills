...
5. **Re-check from the integration worktree** once it exists, before the first dispatch. Services keyed to checkout identity — compose project names, fixed ports, per-repo caches — can be healthy in the user checkout yet broken from a second checkout (a compose project-name collision over one database port is the canonical example). Rerun the availability command from inside the worktree. When a duplicate service would collide with a healthy one, point the worktree at the shared service rather than starting a second one, and record the decision in run state. Sharing is sound while the checks are read-only or cheap; when the checkpoints themselves write to the shared service — one database node serving parallel worktrees is the canonical case, where per-test database setup and cluster-wide role or catalog DDL serialize every suite against every other — prefer a disposable per-worktree service instance (its own compose project, port, and volume) so parallel writers get parallel infrastructure. Where sharing is unavoidable, suite executions take a **suite seat**: at most one or two full-suite or migration-heavy commands running anywhere in the run at a time. Code writing parallelizes; a shared database node does not, and a queue of suites on one is wall time spent at zero throughput.
6. **Measure** what each checkpoint costs: time one focused check and one full configured suite from the integration worktree, abandoning the suite probe after fifteen minutes and recording it as `> 15 min`. Record both numbers in run state as the run's checkpoint budget. They decide schedule shape: a suite over fifteen minutes runs at the final gate and risk-tiered milestones only, and per-ticket smoke stays scoped to the focused suite plus affected components. A preflight that records only availability has learned nothing about the run it is about to schedule.
