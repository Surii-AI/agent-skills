...
6. For logic-bearing outputs (arithmetic, parsing, state transitions), prefer an independent oracle over eyeballing: size it to the ticket's acceptance criteria — a few hand-computed cases per criterion, roughly a dozen cases in total across all criteria. A property sweep beyond that total only when you already suspect a specific defect in a numeric criterion. Never re-derive the implementation's own test matrix. Trust the implementation's tests to verify intent, and your oracle to verify truth.
7. Keep scope proportional to risk. Do not crawl the entire repository or repeat the full suite without a concrete doubt. Run only a targeted command required to confirm or refute a suspected defect. Use the Code locator section's tool for any code lookup rather than crawling.
